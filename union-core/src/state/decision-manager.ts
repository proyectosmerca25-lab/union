...
export class DecisionManager {
  ...
  async approveDecision(decisionId: string): Promise<Decision> {
    ...
    if (decision.status !== 'PROPOSED' && decision.status !== 'REOPENED') {
      throw new InvalidStateTransitionError(
        `Invalid state transition: Cannot approve decision in '${decision.status}' state`
      );
    }
  ...
  async freezeDecision(decisionId: string): Promise<Decision> {
    ...
    if (decision.status !== 'APPROVED') {
      throw new InvalidStateTransitionError(
        `Invalid state transition: Cannot freeze decision in '${decision.status}' state`
      );
    }
  ...
  async rejectDecision(decisionId: string): Promise<Decision> {
    ...
    if (decision.status !== 'PROPOSED' && decision.status !== 'REOPENED') {
      throw new InvalidStateTransitionError(
        `Invalid state transition: Cannot reject decision in '${decision.status}' state`
      );
    }
  ...
  async reopenDecision(decisionId: string, reopenCondition?: string | null): Promise<Decision> {
    ...
    if (decision.status !== 'FROZEN') {
      throw new InvalidStateTransitionError(
        `Invalid state transition: Cannot reopen decision in '${decision.status}' state`
      );
    }
  ...
  async updateDecisionContent(
    ...
    if (decision.status !== 'PROPOSED' && decision.status !== 'REOPENED') {
      throw new InvalidStateTransitionError(
        `Cannot update content of decision in '${decision.status}' state`
      );
    }

    const newTopic = content.topic !== undefined ? content.topic.trim() : decision.topic;
    const newDecision = content.decision !== undefined ? content.decision.trim() : decision.decision;
    const newReason = content.reason !== undefined ? (content.reason ? content.reason.trim() : null) : decision.reason;
    const newAuthority = content.authority !== undefined ? content.authority.trim() : decision.authority;
    const newAlt = content.alternativesConsidered !== undefined
      ...
      : (decision.alternativesConsidered ? JSON.stringify(decision.alternativesConsidered) : null);
  ...
  async supersedeDecision(
    ...
    if (!input || !input.predecessorDecisionId) {
      throw new InvalidInputError('predecessorDecisionId is required');
    }

    const predecessor = await this.getDecision(input.predecessorDecisionId);
    ...
    if (input.sessionId) {
      const session = await this.sessionManager.getSession(input.sessionId);
      if (session.projectId !== predecessor.projectId) {
        throw new CrossProjectViolationError(
          `Cross-project isolation violation: Session '${input.sessionId}' belongs to project '${session.projectId}', not '${predecessor.projectId}'`
        );
      }
    }
    ...
    // Use transaction for multi-record state update
    ...
    try {
      ...
      // Update predecessor to SUPERSEDED
